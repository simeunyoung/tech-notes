...
date: '2025-12-20'
draft: false
summary: '1. 문제 상황 : 커밋이 안된다. Vercel 배포 과정에서 문제가 생긴 줄 알았지만, 실제로는 배포 이전 단계인 git commit 시점에서 작업이 막혔다. - 코드 수정 후 커밋 시도 - 커밋이 정상적으로 완료되지 않음 2. 에러 로그 분석 ESLint: 9.x ConfigError: structuredClone is not defined 왜 발생했을까? 에러 메시지의 핵심인 structuredClone은 자바스크립트의 깊은 복사를 수행하는 최신 API입니다.ESLint 9.x: 최신 업데이트를'
